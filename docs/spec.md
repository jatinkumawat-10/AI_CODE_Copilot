...
- **Recall ≥ 80%** on a labeled set of 30-50 code snippets with planted defects
  (the system must actually flag most planted issues). **First real measurement
  (Stage 2, run 1, n=3 cases × 5 trials, see `eval/EVAL_REPORT.md`): 73.3%
  overall (11/15), but this aggregate hides a sharp split — 100% on
  well-known named vulnerabilities (SQL injection, missing null checks) vs.
  20% on a concurrency/logic bug (race condition).** The 80% target is not
  yet met, and the gap is concentrated in one bug category, not spread
  evenly — a materially different finding than a uniform shortfall would be.
- **Precision ≥ 70%** on the same set (it shouldn't drown real issues in noise).
  **Not yet measurable — no negative (clean-code) test cases exist yet in the
  eval set, so there is no data to compute false-positive rate against.**
- **p95 review latency < 10s** per file/diff at typical PR size. **Measurements
  so far (Stage 2):** case 001 → 29.19s then 5.94s on a re-run (same input,
  different latency — free-tier variance); case 002 → 6.61s then 12.18s;
  case 003 (race condition, harder bug class) → 66.10s with 1140 completion
  tokens, versus ~300 tokens for cases 001/002. Working hypothesis: latency
  scales with output length, and harder-to-reason-about bug classes produce
  longer explanations, hence longer latency. Needs more samples to confirm,
  but this already means the p95 target likely depends heavily on bug
  complexity, not just file size — worth tracking per-category, not as one
  aggregate number, once the eval set is bigger.
